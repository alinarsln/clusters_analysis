...
void My_Search::hierarchical(int k_last) {
    ...
    clusters.resize(all_points.size());
    for(int i=0;i<f.all_points.size();i++)
    {
        min_dist=dist(f.all_points[i],all_points[0]);
        int min_i=0;
        for(int j=0;j<all_points.size();j++)
        {
            if(dist(f.all_points[i],all_points[j])<min_dist)
            {
                min_dist=dist(f.all_points[i],all_points[j]);
                min_i=j;
            }
        }
        clusters[min_i].points.push_back(f.all_points[i]);
    }
}
